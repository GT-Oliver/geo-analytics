...
description: "查询某个网站在 AI 引擎(豆包/DeepSeek/千问/元宝/文心/Kimi 等)里的可见度,以及 AI 带来的站点侧流量与转化。用于回答「我的品牌在 AI 回答里露出多少」「AI 来源带来多少会话和转化」「哪些页面被 AI 引用」这类问题。首次使用会引导用户在浏览器里完成一次授权。"
version: 0.1.0
...
查一个网站在 AI 引擎里的可见度与 AI 来源流量。数据来自该站点自己的埋点与引擎应答采样,
**公开网页上查不到,只能从这个接口取**。
...
429 = 触发限速(每分钟 60 次),响应头 `Retry-After` 告诉你等多久。
